...
class PaperDataUtils:
    ...
    def construct_positive_paper_pairs(self, role, fold):
        ...
        cpapers = utils.load_json_lines(self.paper_dir, cpapers_fname)
        npapers_fname = fname.format('noisy', role, fold)
        npapers = utils.load_json_lines(self.paper_dir, npapers_fname)
    ...
    def dump_paper_pairs(self, paper_pairs, wfname):
        ...
        utils.dump_json(paper_pairs_refactor, self.pairs_dir, wfname)

    # negative pairs num is small compared with positive pairs!!!
    def load_train_neg_paper_pairs(self, fold):
        fname = 'train-neg-pairs-{}.json'.format(fold)
        neg_pairs = utils.load_json(self.pairs_dir, fname)
    ...
    def build_inverted_index(self, fold):
        ...
        papers = utils.load_json_lines(self.paper_dir, fname)
        word2ids = dd(list)
        for paper in papers:
            pid = str(paper['id'])
            title = paper['title']
            words = utils.get_words(title, window=self.ii_window)
            for word in words:
                word2ids[word].append(pid)
        for word in word2ids:
            word2ids[word] = list(set(word2ids[word]))
        utils.dump_json(word2ids, self.inverted_index_dir, 'clean-papers-test-ii-{}.json'.format(fold))
    ...
    def gen_id2papers(self, fold):
        fname = 'clean-papers-test-{}.dat'.format(fold)
        cpapers = utils.load_json_lines(settings.PAPER_DIR, fname)
        id2paper = {}
    ...
    def load_id2papers(self, fold):
        return utils.load_json(self.paper_dir, 'clean-id2paper-test-{}.json'.format(fold))
